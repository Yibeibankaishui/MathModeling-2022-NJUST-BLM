...
\subsection{SLIRS模型}
SLIRS（Susceptible-Lantent-Infectious-Removed-Susceptible）模型，将人群分成了易感人群、病毒携带者、感染者、退出人群。本文模型考虑到了病毒在不同年龄段中的传播特性、致病率、致死率等特性的差异，按14个年龄段划分人群。在人群中展开疫苗接种，能产生一定免疫效果，降低致病率和致死率；与此同时，采用疫苗种类、接种剂数、疫苗免疫效果的时变特性会带来抗病毒效果的差异。因此，本模型同时将。。。考虑抗病毒药物的效果，。。。
\par 采用随机模型，符合二项分布。对每一个个体，其
\subsubsection{年龄段的划分}
\subsubsection{医疗资源}
\subsubsection{药物治疗}
\subsubsection{疫苗}
\subsubsection{群体状态转变的马尔可夫过程}
采用二项分布
